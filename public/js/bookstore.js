(function () {
  ...
  function setupResponsiveTopbar() {
    ...
    // Style auto-porté pour garantir que le bouton hamburger reste invisible
    // sur desktop (>=981px), quelles que soient les CSS de la page hôte.
    // Évite la fine barre blanche aperçue sur les pages sans .menu-toggle CSS.
    if (!document.getElementById("magma-menu-toggle-style")) {
      var st = document.createElement("style");
      st.id = "magma-menu-toggle-style";
      st.textContent =
        '.topbar .menu-toggle{display:none !important;visibility:hidden;width:0;height:0;padding:0;margin:0;border:0;background:transparent;overflow:hidden}' +
        '.topbar .menu-toggle svg{width:0;height:0}' +
        '@media (max-width:980px){' +
          '.topbar .menu-toggle{display:inline-flex !important;visibility:visible;width:auto;height:auto;padding:6px 8px;align-items:center;justify-content:center;border-radius:4px;cursor:pointer;color:#fff;order:2}' +
          '.topbar .menu-toggle svg{width:24px;height:24px;stroke:#fff;fill:none;stroke-width:2}' +
          '.topbar .menu-toggle:hover{background:rgba(255,255,255,.08)}' +
        '}';
